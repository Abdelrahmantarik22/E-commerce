...
import z from "zod";
import { goToOfflinePayment, goToPayment } from "@/app/checkout/check.api/shippingAddress";

export default function Checkk() {
...
const id: string|undefined= cartProducts?.cartId;
...
async function onSubmit(data:z.infer<typeof shoppingSchema>){
 ...
 if(res.status==='success'){
    ...
    const   res =await goToOfflinePayment({
        cartId:id,
        details: data.details,
        phone: data.phone,
        city: data.city
...
<div className="flex items-center gap-8">
            ...
            className="bg-green-800 hover:bg-green-900 dark:text-white cursor-pointer py-2 px-4 mx-auto md:w-4/12 w-full self-center"
            type="submit"
            >
      ...
      <div className={`w-full h-90 overflow-y-auto`}>
        ...
        <span >
            ...
            <div className="flex justify-center items-center  flex-col gap-5  w-full h-[300px]">

<i 
  className="fa-solid fa-wallet -rotate-z-45 text-9xl text-main " 
/>
      <span className="text-[20px] font-bold">You Dont have Product </span>
      <Link className="text-xl font-light hover:text-main transition-colors duration-200" href={'/'}>lets Add some product <i className="fa-solid fa-cart-shopping"></i></Link>
            </div>
            }
    </div>
  );
